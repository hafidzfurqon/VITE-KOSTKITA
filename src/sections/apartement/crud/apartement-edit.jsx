...
export const EditApartement = () => {
    const {id} = useParams()
    const {data, isLoading : loadingData, isFetching : FetchinData} = useFetchDetailApartement(id)
...
const { data: sector = [], isLoading: LoadingSector } = useGetSector(selectedCity);
  ...
  const {mutate, isPending} = useMutationUpdateApartement({
    onSuccess : () => {
      ...
      enqueueSnackbar('Apartement berhasil diupdate', { variant: 'success' });
    },
    onError: () => {
      enqueueSnackbar('Apartement gagal diupdate', { variant: 'error' });
    },
  }, id)
  ...
  if ( isLoading||isFetching || loadingPropertyType || FetchingPropertyType || loadingData || FetchinData) {
    return <Loading/>
  }
  return (
    <Container>
