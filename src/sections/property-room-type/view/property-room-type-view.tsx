...
import { PropertyRoomTypeTableRow } from '../property-room-type-table-row';
import { PropertyRoomTypeTableHead } from '../property-room-type-table-head';
import { TableEmptyRows } from '../table-empty-rows';
import { PropertyRoomTypeTableToolbar } from '../property-room-type-table-toolbar';
...
import {
  useFetchAllPropertyRoomType,
  useMutationCreatePropertyRoomType,
} from 'src/hooks/property-room-type';

export function PropertyRoomTypeView() {
  ...
  const { data = [], isLoading, isFetching } = useFetchAllPropertyRoomType(isOwnerProperty);
  ...
  const { mutate, isPending: isPendingMutate } = useMutationCreatePropertyRoomType({
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['fetch.property-type'] });
      setOpened(false);
      enqueueSnackbar('Tipe Kamar berhasil dibuat', { variant: 'success' });
      reset();
    },
    onError: () => {
      enqueueSnackbar('Tipe Kamar gagal dibuat', { variant: 'error' });
    },
  });
  ...
  return (
    <>
      ...
      <DialogCreate
        ...
        subTitle="Tipe Kamar untuk Ruangan-ruangan di kostkita"
        setOpen={setOpened}
        field={FieldRHF}
