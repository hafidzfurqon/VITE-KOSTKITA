...
import { useMutationDeleteFacilitiesRoom, useMutationUpdateFacilitiesRoom } from 'src/hooks/room-facilities';
...
export function FasilitasTableRow({ row, selected, onSelectRow }: FacilitiesTableRowProps) {
  ...
  const {mutate, isPending : isPendingMutate} = useMutationUpdateFacilitiesRoom({
    onSuccess: () => {
  ...
  const { mutate: DeleteFacilities, isPending } = useMutationDeleteFacilitiesRoom({
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['fetch.facilities.room'] });
      setOpen(false);
